...
void full_coverage(int core, unsigned measurement_time) {
	...
	for(ull sleepphase = sleepphase_start; sleepphase <= sleepphase_max; sleepphase += sleepphase_step) {
		printf("% 10lld", sleepphase);
	}
	printf("\n");

	for(ull loopreps = loopreps_start; loopreps <= loopreps_max; loopreps += loopreps_step) {
